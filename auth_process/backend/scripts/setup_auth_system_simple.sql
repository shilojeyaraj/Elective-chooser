-- Simple Custom Authentication System Setup
...
-- Update profiles table to work with our users table
-- First, let's modify the existing profiles table
DO $$
BEGIN
  -- Check if profiles table exists and modify user_id column
...
-- Drop existing foreign key constraint if it exists and add new one
DO $$
BEGIN
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Could not create user policies: %', SQLERRM;
END $$;
