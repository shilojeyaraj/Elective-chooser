#!/usr/bin/env python3
"""
Script to ingest CS courses from uw_courses_CS.json
"""
...
def ingest_cs_courses(json_file_path: str):
    """Ingest CS courses from JSON file"""
    print(f"📥 Ingesting CS courses from: {json_file_path}")
    ...
    for course_data in courses_data:
        ...
        try:
            ...
            clean_course_data = {
                ...
                'level': int(course_code[2:]) if len(course_code) >= 3 and course_code[2:].isdigit() else 100,
                ...
                'faculty': 'Mathematics',
                'number': int(course_code[2:]) if len(course_code) >= 3 and course_code[2:].isdigit() else 0
            }
    ...
    if failed_count == 0:
        print("🎉 CS courses have been ingested successfully!")
    else:
        print("⚠️ Some courses failed to ingest. Check logs for details.")

if __name__ == "__main__":
    json_file = 'uw_courses_CS.json'
    ingest_cs_courses(json_file)
