#!/usr/bin/env python3
"""
Script to ingest MSE courses from uw_courses_MSE.json
"""
...
def ingest_mse_courses(json_file_path: str):
    """Ingest MSE courses from JSON file"""
    print(f"📥 Ingesting MSE courses from: {json_file_path}")
    ...
    if failed_count == 0:
        print("🎉 MSE courses have been ingested successfully!")
    else:
        print("⚠️ Some courses failed to ingest. Check logs for details.")

if __name__ == "__main__":
    json_file = 'uw_courses_MSE.json'
    ingest_mse_courses(json_file)