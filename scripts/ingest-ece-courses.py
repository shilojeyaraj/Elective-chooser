#!/usr/bin/env python3
"""
Script to ingest ECE courses from uw_courses_ECE_full.json
"""
...
def ingest_ece_courses(json_file_path: str):
    """Ingest ECE courses from JSON file"""
    print(f"📥 Ingesting ECE courses from: {json_file_path}")
    ...
    if failed_count == 0:
        print("🎉 ECE courses have been ingested successfully!")
    else:
        print("⚠️ Some courses failed to ingest. Check logs for details.")

if __name__ == "__main__":
    json_file = 'uw_courses_ECE_full.json'
    ingest_ece_courses(json_file)
