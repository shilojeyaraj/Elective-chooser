#!/usr/bin/env python3
"""
Script to ingest CHE and CHEM courses from uw_courses_CHE_AND_CHEM.json
"""
...
def ingest_che_chem_courses(json_file_path: str):
    """Ingest CHE and CHEM courses from JSON file"""
    print(f"📥 Ingesting CHE and CHEM courses from: {json_file_path}")
    ...
    for course_data in courses_data:
        ...
        try:
            ...
            clean_course_data = {
                ...
                'level': int(course_code[3:]) if len(course_code) >= 4 and course_code[3:].isdigit() else 100,
                ...
                'faculty': 'Engineering' if course_data.get('subject') == 'CHE' else 'Science',
                'number': int(course_code[3:]) if len(course_code) >= 4 and course_code[3:].isdigit() else 0
            }
    ...
    if failed_count == 0:
        print("🎉 CHE and CHEM courses have been ingested successfully!")
    else:
        print("⚠️ Some courses failed to ingest. Check logs for details.")

if __name__ == "__main__":
    json_file = 'uw_courses_CHE_AND_CHEM.json'
    ingest_che_chem_courses(json_file)
