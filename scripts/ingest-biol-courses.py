#!/usr/bin/env python3
"""
Script to ingest BIOL courses from uw_courses_BIOL.json
"""
...
def ingest_biol_courses(json_file_path: str):
    """Ingest BIOL courses from JSON file"""
    print(f"📥 Ingesting BIOL courses from: {json_file_path}")
    ...
    if failed_count == 0:
        print("🎉 BIOL courses have been ingested successfully!")
    else:
        print("⚠️ Some courses failed to ingest. Check logs for details.")

if __name__ == "__main__":
    json_file = 'uw_courses_BIOL.json'
    ingest_biol_courses(json_file)
