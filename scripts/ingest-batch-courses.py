...
def ingest_batch_courses(json_file_path: str):
    print(f"📥 Ingesting batch courses from: {json_file_path}")
    ...
    if failed_count == 0:
        print("🎉 Batch courses have been ingested successfully!")
    else:
        print("⚠️ Some batch courses failed to ingest. Check logs for details.")

    return created_count, updated_count, failed_count

if __name__ == "__main__":
    json_file = 'uw_courses_batch (1).json'
    ingest_batch_courses(json_file)
