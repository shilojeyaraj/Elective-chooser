#!/usr/bin/env python3
"""
Script to ingest remaining courses from uw_courses_remaining.json
"""
...
def ingest_remaining_courses(json_file_path: str):
    """Ingest remaining courses from JSON file"""
    print(f"📥 Ingesting remaining courses from: {json_file_path}")
    ...
    for course_data in courses_data:
        ...
        try:
            ...
            if dept in ['BIOL', 'CHEM', 'PHYS', 'MATH']:
                faculty = 'Science'
            elif dept in ['ECON', 'ENGL', 'PHIL', 'PSYCH']:
                faculty = 'Arts'
            elif dept in ['MATH']:
                faculty = 'Mathematics'
    ...
    if failed_count == 0:
        print("🎉 Remaining courses have been ingested successfully!")
    else:
        print("⚠️ Some courses failed to ingest. Check logs for details.")

if __name__ == "__main__":
    json_file = 'uw_courses_remaining.json'
    ingest_remaining_courses(json_file)
