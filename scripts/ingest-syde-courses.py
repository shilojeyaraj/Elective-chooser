#!/usr/bin/env python3
"""
Script to ingest SYDE courses from uw_courses_SYDE_full.json
"""
...
def ingest_syde_courses(json_file_path: str):
    """Ingest SYDE courses from JSON file"""
    print(f"📥 Ingesting SYDE courses from: {json_file_path}")
    ...
    if failed_count == 0:
        print("🎉 SYDE courses have been ingested successfully!")
    else:
        print("⚠️ Some courses failed to ingest. Check logs for details.")

if __name__ == "__main__":
    json_file = 'uw_courses_SYDE_full.json'
    ingest_syde_courses(json_file)
